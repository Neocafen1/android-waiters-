...
import com.example.neocafewaiterapplication.application.MyApplication
import com.example.neocafewaiterapplication.view.utils.bearerToken
                ...
                initialResponse.code == UNAUTHORIZED_CODE -> {
                    val responseNewTokenLoginModel = runBlocking {
                        localDatabase.fetchRefreshToken()?.let {
                            retrofitAuth().create(RegistrationAPI::class.java).updateAccessTokenWithRefresh(
                                it
                            ).execute()
                        }
                    }
